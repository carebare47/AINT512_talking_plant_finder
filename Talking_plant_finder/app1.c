...
void reverseSearchFromResultsfn(App *app);
...
void freeSpeakfn(App *app) {
	...
	if (firstFreeSpeak == 0) {
		...
		AppAppendTTSPrompt(app, "Which term would you like to add?");
	}
...
int main(int argc, char *argv[])
{
	...
	AppCreateState(app, "reverseSearchFromResults", "root", reverseSearchFromResultsfn);
	
	AppCreateState(app, "results_s2", "root", results_s2_fn);
	AppCreateState(app, "results_s3", "root", results_s3_fn);
	AppCreateState(app, "envPreFixCheck", "root", envPreFixCheckfn);
	...
	//AppGo(app, "reverseSearchFromResults");
	AppGo(app, "askLocale");
	//AppGo(app, "foodfn");
			//AppGo(app, "askLocale");
	//AppGo(app, "freeSpeak");
...
//	randomNumber = rand() % 10;
	...
	AppAppendTTSPrompt(app, " or ");
	SQLspeakQueryResultsOnly(app, "SELECT `Habit` FROM `plantlocations` ORDER BY RAND() LIMIT 1");
	AppAppendTTSPrompt(app, "You can also specify how hardy you want the plant to be.");
	if (!AppRecognize(app)) {
		printf("!AppRec\n");
		return;
	}
...
if (firstCommonNameRun == 0){
	...
	if (NLGetIntSlotValue(AppGetNLResult(app), "common_name_said", &iCommonName) != 119) {
		...
		int localNumMatches = (returnSQLnumberOfMatches(app, "SELECT * FROM composition WHERE `Common Name`='%s'", commonNameArray[iCommonName]) > 2);
		if (localNumMatches > 2) {
	...
	NLGetStringSlotValue(AppGetNLResult(app), "date_time_said", buf, 100);
		if (!strcmp(buf, "date_said")) {
			AppAppendTTSPrompt(app, returnDate());
		}
...
void reverseSearchFromResultsfn(App *app) {
	if (firstCommonNameRun == 0) {
		...
		int confidence;
		AppSetGrammar(app, ".ByName");
		AppAppendTTSPrompt(app, "Please say the common name of the plant you want more information on.");
		if (!AppRecognize(app)) {
			printf("!AppRec\n");
			return;
		}
		...
		RecResultOverallConfidence(AppGetRecResult(app), 0, &confidence);
		NLGetIntSlotValue(AppGetNLResult(app), "common_name_said", &iCommonName);

		printf("String: %s, confidence = %d\n", commonNameArray[iCommonName], confidence);
		if ((NLGetIntSlotValue(AppGetNLResult(app), "common_name_said", &iCommonName) != 119) && (confidence > 95)) {
			AppAppendTTSPrompt(app, commonNameArray[iCommonName]);
			AppAppendTTSPrompt(app, "said.");
			char buf15[200];
			sprintf(buf15, "SELECT * FROM `plantlocations` WHERE `Common name`='%s'  GROUP BY `Latin name`", commonNameArray[iCommonName]);
			...
			iCommonName = -1;
//			AppReturn(app);// (app, "results_s3");

			//AppReturn(app, "results_s3");
			return;
