...
while (x < 2*len(a)/3):
	...
	print("if (%s == 1) {" % a[x])
	print("\tprintf(\"%%s\\n\", \"You said %s.\");" % a[x-1])
	#print("\tplantLocationStringArray[envCounter] = \"%s\";" % a[xTwo])
	print("\tstrcpy(errorEnvStringArray[errorEnvStringCounter], \"%s\");" % a[xTwo])
	print("\terrorEnvStringCounter = errorEnvStringCounter + 1;")
	print("}")
	x = x + 2
	xTwo = xTwo + 1
